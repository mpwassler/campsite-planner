...
import LineString from './map/LineString'
import MapMarker from './map/Marker'
import Map from './map/Map'
import {assignColor} from '../models/colorpicker'
...
export default function ActivityMap(props) {
  ...
  let { activities, lodgings, graph, routes } = props

  let [routeData, setRouteDate] = useState([])
  ...
  return (
    ...
    {!!lodgings && lodgings.map( lodging => {
        return <MapMarker
                    renderMarker={() => (buildLodgingMarker(assignColor()))}
                    key={lodging.name}
                    title={lodging.name}
                    lon={lodging.properties.lon}
                    lat={lodging.properties.lat}/>
      })}
      {activities.map( activity => {
        return <MapMarker
                    ...
                    key={activity.name}
                    title={activity.name}
                    lon={activity.properties.lon}
                    lat={activity.properties.lat}/>
      })}
        {!!routeData && routeData.map( (r, cnt)=> {
            return <LineString
                        name={`item-${cnt}`}
                        route={r}
                        key={cnt}
